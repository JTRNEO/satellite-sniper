...
if __name__ == '__main__':
    ...
    if config.TRAIN.RESUME:
        ...
        arg_params, aux_params = load_param(model_prefix, config.TRAIN.begin_epoch, convert=True)
    else:
    ...
    if config.TRAIN.RESUME:
        mod._preload_opt_states = '%s-%04d.states' % (model_prefix, config.TRAIN.begin_epoch)
    ...
    mod.fit(train_iter, optimizer='sgd', optimizer_params=optimizer_params,
            eval_metric=eval_metrics, begin_epoch=config.TRAIN.begin_epoch, num_epoch=config.TRAIN.end_epoch, kvstore=config.default.kvstore,
