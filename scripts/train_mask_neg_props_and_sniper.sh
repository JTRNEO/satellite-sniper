...
python main_train.py --cfg configs/faster/sniper_res101_e2e.yml --set TRAIN.USE_NEG_CHIPS False TRAIN.ONLY_PROPOSAL True \
...
for cset in "${SETS[@]}"; do
	...
	python main_test.py --cfg configs/faster/sniper_res101_e2e.yml --set TEST.EXTRACT_PROPOSALS True TEST.PROPOSAL_SAVE_PATH ${prop_save_path} \
	TEST.TEST_EPOCH 2 output_path ${prop_train_path} dataset.test_image_set ${cset}
done

echo 'Clearing cache'
rm -r /sniper/data/cache
